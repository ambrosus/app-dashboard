...
AuthRoutes.get('/sessions/:email', AuthController.sessions, (req, res) => { res.status(req.status).json(req.json); });
AuthRoutes.delete('/session/:sessionId', AuthController.session, (req, res) => { res.status(req.status).json(req.json); });
